...
def test_round_artifacts_use_clear_eval_names(tmp_path: Path) -> None:
    ...
    for selector in ["noop", "random"]:
        round_dir = tmp_path / "runs" / "selectors" / selector / "seed_001" / "round_000"
        ...
        assert not (round_dir / "before").exists()
        assert not (round_dir / "after").exists()
